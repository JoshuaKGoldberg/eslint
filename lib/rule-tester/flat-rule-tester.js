...
const
    ...
    // eslint-disable-next-line n/no-restricted-require -- Pre-existing code?
 ...
 * @param {import("eslint").AST.NodeLike} node The node to define.
 * @returns {void}
 */
 ...
 * @param {ParserModule} parser Parser object.
 * @returns {ParserModule} Wrapped parser object.
 */
 ...
 * @template Return
 * @template This
 * @template {string} MethodName
 * @param {MethodName} methodName The name of the method to forbid.
 * @param {{ [i in MethodName]: (...args: Args) => Return }} prototype The prototype with the original method to call.
 * @returns {(...args: Args) => Return} The function that throws the error.
 */
function throwForbiddenMethodError(methodName, prototype) {
    ...
    /**
     * Forwarded method that will throw if called more than once.
     * @param {Args} args Forwarded args for the function.
     * @this {This}
     * @returns {Return} The returned value from the original function.
     * @throws {Error}
     */
...
class FlatRuleTester {
    ...
    static set describe(value) {
        ...
        // @ts-ignore -- Cannot redeclare exported variable '__computed'.
        this[DESCRIBE] = value;
    }
    ...
    run(ruleName, rule, test) {
        ...
        function runRuleForItem(item) {
            ...
            if (schema) {
                ...
                if (ajv.errors) {
                    ...
                    // @ts-ignore -- Is this intentional? Implicit Array joining?
                ...
                try {
                    ...
                    throw new Error(`Schema for rule ${ruleName} is invalid: ${/** @type {Error} */ (err).message}`);
                }
            }
            ...
            try {
                ...
                /** @type {Error} */ (error).message = `ESLint configuration in rule-tester is invalid: ${/** @type {Error} */ (error).message}`;
                throw error;
            }
            ...
            try {
                ...
                forbiddenMethods.forEach(methodName => {
                    SourceCode.prototype[methodName] = throwForbiddenMethodError(methodName, /** @type {any} */ (SourceCode.prototype));
                });
         ...
         * @param {import("eslint").AST.Node} beforeAST AST node before running
         * @param {import("eslint").AST.Node} afterAST AST node after running
         * @returns {void}
         * @private
         */
         ...
         */
        /** @type {any} */ (this.constructor).describe(ruleName, () => {
            if (test.valid.length > 0) {
                /** @type {any} */ (this.constructor).describe("valid", () => {
                    test.valid.forEach(valid => {
                        /** @type {any} */ (this.constructor)[/** @type {ValidTestCase} */ (valid).only ? "itOnly" : "it"](
            ...
            if (test.invalid.length > 0) {
                /** @type {any} */ (this.constructor).describe("invalid", () => {
                    test.invalid.forEach(invalid => {
                        /** @type {any} */ (this.constructor)[invalid.only ? "itOnly" : "it"](
