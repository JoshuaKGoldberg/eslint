...
/** @typedef {import("eslint").ESLint.ResultsMeta} ResultsMeta */
...
// /** @typedef {import("../config/flat-config-array").FlatConfigArray} FlatConfigArrayType */
 ...
 * @param {Map<string,RuleModule>} rules a map of rules from which to generate the object.
...
/** @typedef {Readonly<Readonly<DeprecatedRuleInfo>[]>} ReadonlyDeprecatedRulesInfo */

/** @type {WeakMap<ExtractedConfig, ReadonlyDeprecatedRulesInfo>} */
 ...
 * @returns {ReadonlyDeprecatedRulesInfo} The used deprecated rule list.
 */
...
async function loadFlatConfigFile(filePath) {
     ...
     */
    fileURL.searchParams.append("mtime", /** @type {any} */ (mtime));
    ...
    const config = (await import(/** @type {any} */ (fileURL))).default;
...
/**
 * @typedef {Object} LocatedConfigFile
 * @property {string | undefined} configFilePath Located config file path.
 * @property {string | undefined} basePath Directory containing the config file.
 * @property {Error | null} error Any error found in the process.
 */
 ...
 * @returns {Promise<LocatedConfigFile>} Location information for
 *      the config file.
 */
...
async function calculateConfigArray(eslint, {
    ...
    const configs = new FlatConfigArray(/** @type {FlatConfig[]} */(baseConfig) || [], { basePath, shouldIgnore });
    ...
    if (overrideConfig) {
        if (Array.isArray(overrideConfig)) {
            ...
            configs.push(/** @type {Config} */ (overrideConfig));
        }
    }
