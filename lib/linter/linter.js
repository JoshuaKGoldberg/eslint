...
// PoC: presumably we'd need to find or write a browser-compatible equivalent
                ...
                if (parseResult.success) {
                    Object.keys(parseResult.config).forEach(name => {
                        ...
                        if (
                            ...
                            // PoC: Presumably we'd want some kind of data validity checking
                            const existing = normalizeSeverityToString(config.rules[name][0]);
                            const inline = normalizeSeverityToString(ruleOptions[0]);

                            if (existing === inline) {
                                problems.push(createLintingProblem({
                                    message: `"${name}" is already configured to ${ruleOptions[0]}.\n`,
                                    loc: comment.loc
                                }));
                            }
                    ...
                    for (const { config: inlineConfig, loc } of inlineConfigResult.configs) {
                        ...
                        Object.keys(inlineConfig.rules).forEach(ruleId => {
                            ...
                            try {
                                ...
                                // PoC: Presumably we'd want some kind of data validity checking
                                const existingConfig = config.rules[ruleId];
                                ...
                                if (shouldValidateOptions) {
                                    ...
                                    /**
                                     * If the rule has options configured, check both severity level and those options
                                     */
                                    if (sameSeverity && isDeepStrictEqual(existingConfig.slice(1), ruleOptions.slice(1))) {
                                        inlineConfigProblems.push(createLintingProblem({
                                            ruleId,
                                            message: `"${ruleId}" is already configured to ${ruleOptions[0]} with these options.\n`,
                                            loc,
                                            language: config.language
                                        }));
                                    }
                                    ...
                                    /**
                                     * If the rule has no options configured, purely check the severity level
                                     */
                                    if (sameSeverity) {
                                        inlineConfigProblems.push(createLintingProblem({
                                            ruleId,
                                            message: `"${ruleId}" is already configured to ${ruleOptions[0]}.\n`,
                                            loc,
                                            language: config.language
                                        }));
                                    }
