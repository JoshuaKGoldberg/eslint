...
/** @typedef {import("@eslint/eslintrc").Legacy.CascadingConfigArrayFactory} CascadingConfigArrayFactory */
/** @typedef {import("eslint").ESLint.ConfigData} ConfigData */
/** @typedef {import("eslint").ESLint.Formatter} FormatterFunction */
...
/** @typedef {import("eslint").Linter.StringSeverity} StringSeverity */
/** @typedef {import("eslint").Linter.SuppressedLintMessage} SuppressedLintMessage */
/** @typedef {import("eslint").Rule.DeprecatedRuleInfo} DeprecatedRuleInfo */
 ...
 * @property {string[]|null} [envs] An array of environments to load.
 * @property {string[]|null} [extensions] An array of file extensions to check.
 * @property {boolean|((message: LintMessage) => boolean)} [fix] Execute in autofix mode.
 ...
 * @property {Record<string,RuleLevel>} [rules] An object of rules to use.
 * @property {string[]} [rulePaths] An array of directories to load custom rules from.
 * @property {boolean | StringSeverity} [reportUnusedDisableDirectives] `true`, `"error"` or '"warn"' adds reports for unused eslint-disable directives
 ...
 * @returns {Omit<LintReport, 'results' | 'usedDeprecatedRules'>} Contains the stats
 * @private
 */
...
/**
 * @typedef {import("../linter/linter").Linter} Linter
 */
 ...
 * @param {boolean | ((message: LintMessage) => boolean)} config.fix If `true` or a function that returns true then it does fix.
 * @param {boolean} config.allowInlineConfig If `true` then it uses directive comments.
 * @param {boolean | StringSeverity} config.reportUnusedDisableDirectives If `true`, `"error"` or '"warn"', then it reports unused `eslint-disable` comments.
 ...
 * @returns {RuleModule|null} The rule or null.
 */
 ...
 * @param {Set<string>} fixTypes An array of fix types to check.
 ...
 * @returns {Record<string,boolean> | undefined} The boolean map.
 */
...
function directoryExists(resolvedPath) {
    try {
        ...
        if (error && (/** @type {NodeJS.ErrnoException} */ (error).code === "ENOENT" || /** @type {NodeJS.ErrnoException} */ (error).code === "ENOTDIR")) {
            return false;
        }
        throw error;
    }
    ...
    executeOnFiles(patterns) {
        ...
        if (!cache) {
            try {
                ...
                const errorCode = error && /** @type {NodeJS.ErrnoException} */ (error).code;
        ...
        if (typeof resolvedFormatName === "string") {
            ...
            try {
                ...
                if (removedFormatters.has(format)) {
                    /** @type {Error} */ (ex).message = `The ${format} formatter is no longer part of core ESLint. Install it manually with \`npm install -D eslint-formatter-${format}\``;
                } else {
                    /** @type {Error} */ (ex).message = `There was a problem loading formatter: ${formatterPath}\nError: ${/** @type {Error} */ (ex).message}`;
                }
                throw ex;
            }
