...
/** @typedef {import("@humanwhocodes/config-array").ObjectPropertySchema} ObjectPropertySchema */

//-----------------------------------------------------------------------------
// Helpers
//-----------------------------------------------------------------------------

const ruleSeverities = new Map(/** @type {[number | string, number][]} */ ([
    [0, 0], ["off", 0],
    [1, 1], ["warn", 1],
    [2, 2], ["error", 2]
]));
    ...
    merge(first = {}, second = {}) {
        ...
        for (const ruleId of Object.keys(result)) {
            ...
            try {
                ...
                throw new Error(`Key "${ruleId}": ${/** @type {any} */ (ex).message}`, { cause: ex });
            }
