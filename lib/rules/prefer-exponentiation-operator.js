 ...
 * @param {import("eslint").AST.Node} base The node to check.
 ...
 * @param {import("eslint").AST.Node} exponent The node to check.
 * @returns {boolean} `true` if the node needs to be parenthesised.
 */
 ...
 * @param {import("eslint").AST.Node} node A node that would be replaced by an exponentiation binary expression.
...
module.exports = {
    ...
    create(context) {
         ...
         * @param {import("eslint").AST.Node} node 'Math.pow()' node to report.
         * @returns {void}
         */
