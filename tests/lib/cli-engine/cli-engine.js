...
describe("CLIEngine", () => {
    ...
    describe("executeOnText()", () => {
        ...
        describe("when using local cwd .eslintrc", () => {
            ...
            it("should report the total and per file errors", () => {
                ...
                assert.strictEqual(
                    report.results[0].messages[0].ruleId,
                    "strict",
                );
                assert.strictEqual(
                    report.results[0].messages[1].ruleId,
                    "no-var",
                );
                assert.strictEqual(
                    report.results[0].messages[2].ruleId,
                    "no-unused-vars",
                );
                assert.strictEqual(
                    report.results[0].messages[3].ruleId,
                    "quotes",
                );
                assert.strictEqual(
                    report.results[0].messages[4].ruleId,
                    "eol-last",
                );
                assert.strictEqual(report.results[0].fixableErrorCount, 3);
                assert.strictEqual(report.results[0].fixableWarningCount, 0);
                assert.strictEqual(
                    report.results[0].suppressedMessages.length,
                    0,
                );
            });
        ...
        describe("(plugin shorthand)", () => {
            ...
            it("should resolve 'plugins:[\"@scope\"]' to 'node_modules/@scope/eslint-plugin'.", () => {
                ...
                const report = engine.executeOnText("var x = 0", "index.js")
                    .results[0];
    ...
    describe("executeOnFiles()", () => {
        ...
        it("should return 3 messages when given a config file and a directory of 3 valid files", () => {
            ...
            assert.strictEqual(
                path.relative(fixturePath, report.results[0].filePath),
                "async.js",
            );
            assert.strictEqual(report.results[0].errorCount, 0);
            assert.strictEqual(report.results[0].warningCount, 0);
            ...
            assert.strictEqual(report.results[0].suppressedMessages.length, 0);
            assert.strictEqual(
                path.relative(fixturePath, report.results[1].filePath),
                "broken.js",
            );
            ...
            assert.strictEqual(report.results[1].suppressedMessages.length, 0);
            assert.strictEqual(
                path.relative(fixturePath, report.results[2].filePath),
                "cwd.js",
            );
            ...
            assert.strictEqual(
                path.relative(fixturePath, report.results[3].filePath),
                "simple.js",
            );
            ...
            assert.strictEqual(
                path.relative(fixturePath, report.results[4].filePath),
        ...
        describe("cache", () => {
                ...
                it("should create the cache file inside the provided directory", () => {
                    ...
                    assert.isTrue(
                        shell.test(
                            "-f",
                            path.resolve(
                                `./tmp/.cacheFileDir/.cache_${hash(process.cwd())}`,
                            ),
                        ),
            ...
            it("should create the cache file inside cwd when no cacheLocation provided", () => {
                ...
                assert.isTrue(
                    shell.test("-f", path.resolve(cwd, ".eslintcache")),
                    "the cache for eslint was created at provided cwd",
                );
            });

            it("should invalidate the cache if the configuration changed between executions", () => {
                assert.isFalse(
                    shell.test("-f", path.resolve(".eslintcache")),
                    "the cache for eslint does not exist",
                );
                ...
                assert.strictEqual(
                    ...
                    0,
                    "the file passed without errors or warnings",
                );
                assert.strictEqual(
            ...
            it("should store in the cache a file that failed the test", () => {
                ...
                assert.isTrue(
                    ...
                    "the entry for the good file is in the cache",
                );

                assert.isTrue(
                    typeof cache.getKey(badFile) === "object",
                    "the entry for the bad file is in the cache",
                );
            ...
            it("should not contain in the cache a file that was deleted", () => {
                ...
                assert.isTrue(
                    typeof cache.getKey(toBeDeletedFile) === "object",
                    "the entry for the file to be deleted is in the cache",
                );
                ...
                assert.isTrue(
                    typeof cache[toBeDeletedFile] === "undefined",
                    "the entry for the file to be deleted is not in the cache",
                );
            });
            ...
            it("should contain files that were not visited in the cache provided they still exist", () => {
                ...
                assert.isTrue(
                    typeof cache.getKey(testFile2) === "object",
                    "the entry for the test-file2 is in the cache",
                );
            ...
            describe("cacheFile", () => {
                it("should use the specified cache file", () => {
                    ...
                    const cachedResult = engine.executeOnFiles([
                        badFile,
                        goodFile,
                    ]);
            ...
            describe("cacheStrategy", () => {
                it("should detect changes using a file's modification time when set to 'metadata'", () => {
                    ...
                    assert.isFalse(
                        fileCache.getFileDescriptor(badFile).changed,
                        `the entry for ${badFile} is unchanged`,
                    );
                    assert.isTrue(
                        fileCache.getFileDescriptor(goodFile).changed,
        ...
        describe("overrides", () => {
            ...
            it("should recognize dotfiles", () => {
                ...
                assert.strictEqual(
                    ret.results[0].messages[0].ruleId,
                    "no-unused-vars",
                );
        ...
        describe("multiple processors", () => {
            const root = path.join(
                os.tmpdir(),
                "eslint/cli-engine/multiple-processors",
            );
            const commonFiles = {
    ...
    describe("isPathIgnored", () => {
        ...
        describe("about the default ignore patterns", () => {
            ...
            it("should allow subfolders of defaultPatterns to be unignored by ignorePattern", () => {
                ...
                const engine = new CLIEngine({
                    cwd,
                    ignorePattern: "!/node_modules/package",
                });
    ...
    describe("getErrorResults()", () => {
        ...
        it("should report no suppressed error messages when looking for errors only", () => {
            ...
            const report = engine.executeOnText(
                "var foo = 'bar'; // eslint-disable-line quotes -- justification\n",
            );
            const errorResults = CLIEngine.getErrorResults(report.results);
    ...
    describe("outputFixes()", () => {
        ...
        it("should call fs.writeFileSync() for each result with output", () => {
            ...
            assert.isTrue(
                spy.firstCall.calledWithExactly("foo.js", "bar"),
                "First call was incorrect.",
            );
            assert.isTrue(
                spy.secondCall.calledWithExactly("bar.js", "baz"),
    ...
    describe("getRules()", () => {
        ...
        it("should expose the list of plugin rules", () => {
            ...
            assert(
                engine.getRules().has("eslint-plugin/require-meta-schema"),
                "eslint-plugin/require-meta-schema is present",
            );
        });
        ...
        it("should expose the list of rules from a preloaded plugin", () => {
            ...
            assert(
                engine.getRules().has("foo/no-invalid-meta"),
                "foo/no-invalid-meta is present",
            );
        });
    });
    ...
    describe("resolveFileGlobPatterns", () => {
        ...
        it("should convert a directory name with no provided extensions into a glob pattern", () => {
            ...
            const result = new CLIEngine(opts).resolveFileGlobPatterns(
                patterns,
            );
    ...
    describe("with ignorePatterns config", () => {
        ...
        describe("ignorePatterns can add an ignore pattern ('foo.js').", () => {
            ...
            it("'isPathIgnored()' should return 'false' for 'bar.js'.", () => {
                ...
                assert.strictEqual(
                    engine.isPathIgnored("subdir/bar.js"),
                    false,
                );
            });
        ...
        describe("ignorePatterns can unignore '/node_modules/foo'.", () => {
            ...
            it("'isPathIgnored()' should return 'false' for 'node_modules/foo/index.js'.", () => {
                ...
                assert.strictEqual(
                    engine.isPathIgnored("node_modules/foo/index.js"),
                    false,
                );
            });
            ...
            it("'isPathIgnored()' should return 'true' for 'node_modules/foo/.dot.js'.", () => {
                ...
                assert.strictEqual(
                    engine.isPathIgnored("node_modules/foo/.dot.js"),
                    true,
                );
            });
            ...
            it("'isPathIgnored()' should return 'true' for 'node_modules/bar/index.js'.", () => {
                ...
                assert.strictEqual(
                    engine.isPathIgnored("node_modules/bar/index.js"),
                    true,
                );
            });
        ...
        describe("ignorePatterns in the config file in a child directory affects to only in the directory.", () => {
            ...
            it("'isPathIgnored()' should return 'true' for 'foo.js'.", () => {
                ...
                assert.strictEqual(engine.isPathIgnored("subdir/foo.js"), true);
                assert.strictEqual(
                    engine.isPathIgnored("subdir/subsubdir/foo.js"),
                    true,
                );
            });
            ...
            it("'isPathIgnored()' should return 'true' for 'bar.js' in 'subdir'.", () => {
                ...
                assert.strictEqual(
                    engine.isPathIgnored("subdir/subsubdir/bar.js"),
                    true,
                );
            });
        ...
        describe("ignorePatterns in the config file in a child directory can unignore the ignored files in the parent directory's config.", () => {
            ...
            it("'isPathIgnored()' should return 'false' for 'foo.js' in the child directory.", () => {
                ...
                assert.strictEqual(
                    engine.isPathIgnored("subdir/foo.js"),
                    false,
                );
            });
