    ...
    describe("applyFixes() with no BOM", () => {
        describe("shouldFix parameter", () => {
            ...
            it("should only apply fixes for which the 'shouldFix' function returns true", () => {
                const shouldFixSpy = sinon.spy(
                    (problem) => problem.message === "foo",
                );
                const result = SourceCodeFixer.applyFixes(
                    TEST_CODE,
                    [INSERT_AT_START, REPLACE_ID],
                    shouldFixSpy,
                );
        ...
        describe("Text Insertion", () => {
            ...
            it("should insert text at the beginning of the code", () => {
                ...
                assert.strictEqual(
                    result.output,
                    INSERT_AT_START.fix.text + TEST_CODE,
                );
                assert.strictEqual(result.messages.length, 0);
            });
        ...
        describe("Combination", () => {
            ...
            it("should apply the same fix when ranges overlap regardless of order", () => {
                const result1 = SourceCodeFixer.applyFixes(TEST_CODE, [
                    REMOVE_MIDDLE,
                    REPLACE_ID,
                ]);
                const result2 = SourceCodeFixer.applyFixes(TEST_CODE, [
                    REPLACE_ID,
                    REMOVE_MIDDLE,
                ]);
    ...
    describe("applyFixes() with BOM", () => {
        ...
        describe("Text Insertion", () => {
            ...
            it("should insert text at the beginning, middle, and end of the code", () => {
                ...
                const insertInMiddle = TEST_CODE.replace(
                    "6 *",
                    `${INSERT_IN_MIDDLE.fix.text}6 *`,
                );
                ...
                assert.strictEqual(
                    result.output,
                    `\uFEFF${INSERT_AT_START.fix.text}${insertInMiddle}${INSERT_AT_END.fix.text}`,
                );
                assert.strictEqual(result.messages.length, 0);
            });
        ...
        describe("Combination", () => {
            ...
            it("should apply the same fix when ranges overlap regardless of order", () => {
                const result1 = SourceCodeFixer.applyFixes(TEST_CODE_WITH_BOM, [
                    REMOVE_MIDDLE,
                    REPLACE_ID,
                ]);
                const result2 = SourceCodeFixer.applyFixes(TEST_CODE_WITH_BOM, [
                    REPLACE_ID,
                    REMOVE_MIDDLE,
                ]);
